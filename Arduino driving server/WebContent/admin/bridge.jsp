	...
	<ADS:ifPerm permission="bridge" minValue="ALL">
		<script>
			...
			function closeBridge(){
				...
				ADS.changeBridgeState(false, function(answ, err){
			...
			function openBridge(){
				...
				ADS.changeBridgeState(true, function(answ, err){
	...
	if(request.getParameter("edit") == null){
				...
				<div class="col-md-12">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							if(Bridge.isOpened()){
								%>
								Bridge is opened. <button type="button" class="btn btn-default btn-xs"
									onclick="closeBridge()" title="Close the bridge.">Close</button><br>
								Count of different bridges : <%= Bridge.getBridges().size() %><br>
								Count of different hardwares : <%= Bridge.getIFaces().size() %><br>
								<%
							}else{
								%>
								Bridge isn't opened. <button type="button" class="btn btn-default btn-xs"
									...
									for(AbstractBridgeInterface iface : ifaces){
										...
										<div class="col-md-6">
											<div class="panel panel-default">
												...
												<div class="panel-body">
													<strong>HID :</strong> <%= iface.getHID().hid %><br>
													<strong>Name :</strong> <%= iface.getHID().name %><br>
													<strong>Creator :</strong> <%= iface.getHID().creator %><br>
													<strong>Port :</strong> <%= iface.getPortName() %><br>
													<strong>Bridge type :</strong> <%=iface.getBridgeName()%><br>
												</div>
											</div>
										</div>
										<%
									...
									for(AbstractBridge bridge : bridges){
										...
										<div class="col-md-6">
											<div class="panel panel-default">
												...
												<div class="panel-body">
													<strong>activated :</strong> <%= bridge.isActivated() %><br>
													<strong>name :</strong> <%= bridge.getName() %><br>
		...
		<ADS:ifPerm permission="bridge" minValue="ALL" invert="true">
			<div class="alert alert-danger" role="alert">
				...
				You aren't allowed to edit Bridge !
			</div>
		</ADS:ifPerm>
		<ADS:ifPerm permission="bridge" minValue="ALL">
			<%
			if(request.getParameter("action") == null){
				...
				if(request.getParameter("bridge") == null){
					...
					<div class="alert alert-danger" role="alert">
						<span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>
						<span class="sr-only">Error:</span>
						There is any bridge named <%= request.getParameter("bridge") %>
					</div>
					<%
				}else{
					...
					%>
					<div class="panel panel-default">
						<h1 class="panel title">Edit bridge <%= bridge.getName() %></h1>
						<div class="panel-body">
							<div class="form-group">
								<label for="bridge_name">Name</label>
								<input type="text" class="form-control" placeholder="Name" id="bridge_name"
									aria-describedby="basic-addon2" value="<%= bridge.getName() %>"/>
							</div>
							<div class="form-group">
								<label for="bridge_desc">Description</label>
								<textarea id="bridge_desc" class="form-control" placeholder="Description"
									aria-describedby="basic-addon2"><%= bridge.getDesc() %></textarea>
							</div>
							<div class="checkbox">
								<label>
									<input <% if(bridge.isActivated())out.print("checked"); %>
