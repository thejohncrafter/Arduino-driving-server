...
public class EditServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		this.getServletContext().getRequestDispatcher("/ADS")
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
...

			{
				...
				if (user == null) {
					...
					return;
						...
						.equals(object.getJSONObject("user").getString(
								"password"))) {
			...
			case "Users.getInstance().new": {

				String name = StringEscapeUtils.escapeHtml(object
						.getJSONObject("newUser").getString("name"));
				String password = StringEscapeUtils.escapeHtml(object
						.getJSONObject("newUser").getString("password"));
				String group = StringEscapeUtils.escapeHtml(object
						.getJSONObject("newUser").getString("group"));
				...
				if (Users.getInstance().getUser(name) != null) {

					System.out.println("Error : a user is already named "
							+ name);
				...
				} else if (!group.equals("sudo")
						&& Permissions.getInstance().getGroup(group) == null) {
					...
					try {
						...
						Users.getInstance().newUser(name, password, group);

					} catch (JDOMException e) {

						System.out.println("Error when creating user :");
						e.printStackTrace();
						out.print(stringer.object().key("error")
								.value("Internal error : " + e.getMessage())
								.endObject().toString());
				...
				break;
			case "Users.getInstance().edit": { // for variables

				String name = StringEscapeUtils.escapeHtml(object
						.getString("oldUser"));
				String newName = StringEscapeUtils.escapeHtml(object
						.getJSONObject("newUser").getString("name"));
				String newPassword = StringEscapeUtils.escapeHtml(object
						.getJSONObject("newUser").getString("password"));
				String newGroup = StringEscapeUtils.escapeHtml(object
						.getJSONObject("newUser").getString("group"));
				...
				if (Users.getInstance().getUser(name) == null) {
					...
					out.print("{\"error\":\"Atempt to rename sudo !\"}");
						...
						&& Users.getInstance().getUser(newName) != null) {
				...
				} else if (!newGroup.equals("sudo")
						&& Permissions.getInstance().getGroup(newGroup) == null) {
					...
					try {
						...
						Users.getInstance().changeUsername(name, newName);
						Users.getInstance().setPassword(newName, newPassword);
						Users.getInstance().changeGroup(newName, newGroup);
				...
				break;
			case "Users.getInstance().remove": { // for variables

				String user = StringEscapeUtils.escapeHtml(object
						.getString("toRemove"));
			...
			case "bridge.open":
				if (Bridge.getInstance().isOpened()) {
					...
					try {
						...
						Element rootNode = ArduinoDriving
								.getConfigElement("bridge");
						...
						output.output(
								...
								new FileOutputStream(ArduinoDriving
										.getRealPath("WEB-INF/bridge.xml")));
			...
			case "bridges.edit": {
				...
				String name = StringEscapeUtils.escapeHtml(newDatas
						.getString("name"));
				String desc = StringEscapeUtils.escapeHtml(newDatas
						.getString("desc"));
				...
				if (Bridge.getInstance().getBridges().get(oldName) == null) {
					...
					break;
						...
						&& Bridge.getInstance().getBridges().get(name) != null) {

					System.out
							.println("Error : the given name already exists !");
					...
					System.out
							.println("Error : the given description is blank !");
					...
					try {
						...
						File bridgesfile = new File(
								ArduinoDriving
										.getRealPath("WEB-INF/bridge.xml"));
						SAXBuilder builder = new SAXBuilder();
						Document document = (Document) builder
								.build(bridgesfile);
						List<Element> bridges = document.getRootElement()
								.getChild("bridges").getChildren();

						for (int i = 0; i < bridges.size(); i++) {

							if (bridges.get(i).getAttributeValue("name")
									.equals(oldName)) {
					...
					AbstractBridge bridge = Bridge.getInstance().getBridges()
							.get(oldName);
					...
					if (activated != bridge.isActivated()) {
						...
						if (activated) {
							...
							Bridge.getInstance().initBridge(bridge);

						} else {

							try {

								Bridge.getInstance().destroyBridge(bridge);
					...
					bridge.setDesc(desc);
					Bridge.getInstance().changeBridgeName(bridge, name);
					Bridge.getInstance()
							.getBridges()
							.put(name,
									Bridge.getInstance().getBridges()
											.remove(oldName));
			...
			case "groups.new": {
				...
				String name = StringEscapeUtils.escapeHtml(object
						.getJSONObject("group").getString("name"));
				JSONObject perms = object.getJSONObject("group").getJSONObject(
						"perms");
				...
				if (Permissions.getInstance().getGroup(name) != null) {
					...
					for (String key : keys) {
						...
						case "NONE":
							break;
						default:
							out.print("{\"error\":\"Bad value for permission "
									+ StringEscapeUtils.escapeJavaScript(key)
									+ " !\"}");
							break main;
						}
					...
					try {
						...
						Permissions.getInstance().createGroup(name, permsMap);

					} catch (JDOMException e) {

						System.out.println("Error :");
						e.printStackTrace();
						out.print(new JSONStringer().object().key("error")
								.value("Internal error : " + e.getMessage())
								.endObject().toString());
						break;
			...
			case "groups.edit": {
				...
				String oldName = object.getJSONObject("group").getString(
						"oldName");
				String name = StringEscapeUtils.escapeHtml(object
						.getJSONObject("group").getString("name"));
				JSONObject perms = object.getJSONObject("group").getJSONObject(
				...
				if (Permissions.getInstance().getGroup(oldName) == null) {
				...
				if (!oldName.equals(name)
						&& Permissions.getInstance().getGroup(name) != null) {
				...
				if (name.length() == 0 || name.trim().length() == 0) {
					...
					try {
						...
						Permissions.getInstance().editGroup(oldName, name,
								permsMap);
			...
			case "groups.remove":
				...
				if (Permissions.getInstance().getGroup(name) == null) {
				...
				} else if (Users.getInstance().getUsersByGroup(name).length != 0) {
					...
					try {
						...
						Permissions.getInstance().removeGroup(name);
