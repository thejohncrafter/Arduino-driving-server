...
/**
 * This singleton contains the main part of the plugins system.
 * 
...
public class Plugins {
	...
	/**
	 * This method loads all the plugins from the folder named
	 * <code>plugins</code>.
	 * 
	 * @throws PluginException
	 *             If an excaption occurs.
	 */
	private void load() throws PluginException {
		...
		try {
			...
			File[] pluginJars = new File(ArduinoDriving.getRealPath("plugins"))
					.listFiles();
			...
			for (File pluginJar : pluginJars) {
				...
				try {

					{
						...
						File f = new File(pluginJar.getAbsolutePath()
								+ "/plugin.jar");

						if (f.exists())
							zip = new ZipFile(f);
						else
							throw new PluginException("Can't load plugin "
									+ pluginJar.getName()
									+ " : missing plugin.jar !");
					...
					Document document = builder.build(zip.getInputStream(zip
							.getEntry("ADS-plugin.xml")));
					...
					System.err.println("Can't load plugin "
							+ pluginJar.getName() + " !");
					e.printStackTrace();
	 ...
	 * 
	 * @param content
	 *            The content element of the plugin.
	 * @param archive
	 *            The plugin's archive.
	 * @throws IOException
	 *             If an I/O exception occurs.
	 * @throws PluginException
	 *             If another exception occurs.
	 */
	private void loadContent(Element[] content, ZipFile archive)
		...
		for (Element e : content) {
			...
			if (e.getName().equals("driver")) {
				...
				Element[] files = e
						...
						.getChildren()
						.toArray(
								new Element[e.getChild("files").getChildren()
										.size()]);
				...
				for (Element file : files) {
					...
					System.out.println("Getting file from " + src
							+ " and putting to " + out + "...");
					InputStream is = archive.getInputStream(archive
							.getEntry(src));
					new File(ArduinoDriving.getRealPath("drivers/"
							+ hid.replace(" ", "_") + out)).createNewFile();
					FileOutputStream os = new FileOutputStream(new File(
							ArduinoDriving.getRealPath("drivers/"
									+ hid.replace(" ", "_") + out)));
	 ...
	 * 
	 * @param archive
	 *            The folder where the plugin is stored.
	 * @param cfg
	 *            The bridge configuration element.
	 * @throws PluginException
	 *             If an error occurs.
	 */
	private void loadBridge(File archive, Element cfg) throws PluginException {
		...
		if (name == null)
			throw new PluginException("Can't load a bridge from plugin "
					+ archive.getName() + " : missing attribute name !");

		if (classname == null)
			throw new PluginException("Can't load bridge " + name
					+ " from plugin " + archive.getName()
					+ " : missing classname !");
		...
		try {
			...
			throw new PluginException("Can't load bridge " + name
					+ " from plugin " + archive.getName()
					+ " : Can't instantiate " + bridgeClass.getName(), e);
		...
		bridge.setDesc(desc);
		bridge.setId(id);
		bridge.setActivated(Bridge.getInstance().isOpened());
		Bridge.getInstance().getBridges().put(id, bridge);
	 ...
	 * 
	 * @return The instance if it exists, else it creates then returns it.
	 */
	public static Plugins getInstance() {

		return instance != null ? instance : (instance = new Plugins());
